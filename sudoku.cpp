...
Hours spent: 1
Any difficulties?: no you gave us pseduo code
Complexity of this algorithm: O(n^m) n is the number of possible inputs aka 1-4/1-9. and m is the number of empty squares
***************************************************/
#include <iostream>
	...
	if(s[r][c]==0)//If the current slot is 0, look for a number that works
	  {
	    for(int k=1;k<=SIZE;k++) //a number can be 1 through SIZE
	      {
		if(valid(s,r,c,k)) //call valid to see if the current number is possible
		  {
		    s[r][c]=k; //possible, then put the current number into the current slot.
